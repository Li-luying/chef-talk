...
## `culture-query` - 知识库查询（仅限宽泛饮食文化）
⚠️ 此类仅用于知识库中有专门长文记载的宽泛话题，**绝大多数问题不应该选这个**。
典型场景（很少用到）：
- 菜系宏观发展历史（如"鲁菜的发展历史"）
- 名厨生平、地域饮食文化演变
- 饮食文化专题（如"中国茶文化"）

## `recipe-query` - 图谱查询（做法步骤、口味特点、菜系介绍、具体菜品典故）
当问题涉及**任何具体菜品**（做法、食材、历史典故、口味）或菜系特点时，必须使用此类。
请在以下情况毫不犹豫地选择 `recipe-query`：
- 询问菜谱的做法、步骤细节（如"红烧肉怎么做"）
- 询问具体菜品的历史典故（如"宫保鸡丁的历史典故"、"鱼香肉丝的由来"）
- 询问食材用量、所需原料（如"宫保鸡丁需要哪些食材"）
- 询问烹饪方法、技巧（如"糖醋排骨用什么烹饪方法"）
- 询问菜系的口味特点和代表菜品（如"川菜有什么特点"、"粤菜的口味风格"）
- 询问某菜系有哪些经典菜品（如"川菜有哪些代表菜"）
...
**重要提示**：
1. 对于"有多少"、"统计"、"数量"这类问题，务必选择 `stats-query`
2. 只要问题提到了一道**具体的菜名**（无论问做法、食材、历史还是典故），一律选择 `recipe-query`
3. 对于"XX菜的特点/口味"、"XX菜系"这类问题，选择 `recipe-query`
4. `culture-query` 基本不用，除非问题完全不涉及具体菜品、纯粹是宽泛的饮食文化史
5. 请准确输出类型字符串，不要输出其他内容或添加额外字段

---
## 输出前自检（每次分类前心里过一遍）
1. 用户问题中是否提到了具体菜名？→ 是：必须选 `recipe-query`
2. 用户是否在问"怎么做"、"食材"、"口味"、"历史典故"？→ 是：必须选 `recipe-query`
3. 用户是否在问统计数字（多少、排名、占比）？→ 是：必须选 `stats-query`
4. 这是否是一个宽泛的饮食文化问题、完全不涉及具体菜品？→ 是：才选 `culture-query`
5. 我的选择是否经得起上述 4 条的检验？→ 如果有任何疑虑，优先选 `recipe-query`
...
记住：
- **凡是与“做法、步骤、火候、食材、用什么、怎么做”相关的问题，默认选择 `recipe-query`。**
